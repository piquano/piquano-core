...
DELTA_SELECT_FIELDS = ','.join([
    ...
    'from', 'sender', 'toRecipients', 'ccRecipients', 'replyTo',
    ...
    for msg in page.get('value') or []:
        ...
        try:
            created, updated = adapter.persist_message(
                account=account, msg=msg, folder=folder,
                matched_entities=matched, client=client, dry_run=dry_run,
            )
        except Exception:
    ...
    *,
    ...
    try:
        while next_url:
            ...
            _process_page(
                account=account, folder=folder, page=page,
                result=result, client=client, adapter=adapter, dry_run=dry_run,
            )
        ...
        for folder in ('Inbox', 'SentItems'):
            try:
                results[folder] = run_folder_sync(
                    account, folder=folder, backfill_days=backfill_days,
                    dry_run=dry_run, full_resync=full_resync, adapter=adapter,
                )
        ...
        if sync_log is not None:
            ...
            adapter.update_sync_log(
                ...
                details={
                    ...
                    'totals': totals,
                    'per_folder': {
                        f: {'fetched': r.fetched, 'matched': r.matched,
                            'new': r.persisted_new, 'updated': r.persisted_updated,
                            'skipped': r.skipped_no_match, 'failed': r.failed}
